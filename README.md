...
2. Run `roslaunch drv_brain drv_v2.launch` or `roslaunch drv_brain drv_v2_simple.launch` to launch the whole robot vision system on a single machine. The whole vision system runs in namespace /vision, and you can change it by modifing the arg "ns" in drv_v2.launch. Meanwhile, you need pay attention to the launch file drv_v2_host.launch.xml, in which we assume your Arduino board link to USB port "/dev/Arduino_vision", you can make this symlink by following [this instruction](http://www.joakimlinde.se/microcontrollers/arduino/avr/udev.php). Briefly, you need create a udev rule file for Arduino and put it into the folder `/etc/udev/rules.d`. The main difference of the two launch files is the simple version doesn't perform searching and uses KCF tracker.
